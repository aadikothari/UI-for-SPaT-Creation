...
class EXTRTCLASS ASN1MessageBuffer : public OSRTMessageBuffer
{
 ...
 public:
   ...
   /**
    * Add the given event handler to this object's context, replacing any
    * previous event handler. This does not take ownership of the handler.
    */
   void addRawEventHandler(Asn1RawEventHandler* pHandler) {
      pHandler->addEventHandler(getCtxtPtr());
   }
   ...
   void removeRawEventHandler() 
      { Asn1RawEventHandler::removeEventHandler(getCtxtPtr()); }
...
class EXTRTCLASS ASN1CType {
 ...
 public:
   ...
   /**
    * The \c MemFree method invokes the asn1Free* function on the message using
    * this object's OSCTXT.
    * The implementation here does nothing.  Subclasses must override this
    * method to invoke the correct asn1Free function when there is one (the
    * function is only generated when required).
    */
